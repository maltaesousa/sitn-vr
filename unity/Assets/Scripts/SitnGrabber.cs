...
//-------------------------------------------------------------------------
public class SitnGrabber : MonoBehaviour
{
    ...
    public void AttachObject(GameObject objectToAttach, GrabTypes grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
    {
        ...
        if (attachedObject.attachedRigidbody != null)
        {
            if (attachedObject.interactable.attachedToHand != null) //already attached to another hand
            {
                ...
                {
                    Hand.AttachedObject attachedObjectInList = attachedObject.interactable.attachedToHand.AttachedObjects[attachedIndex];
    ...
    {
        SitnControllerButtonHints.ShowButtonHint(this, grabGripAction); //todo: assess
    }

    public void HideGrabHint()
    {
        SitnControllerButtonHints.HideButtonHint(this, grabGripAction); //todo: assess
    }

    public void ShowGrabHint(string text)
    {
        SitnControllerButtonHints.ShowTextHint(this, grabGripAction, text);
    }
